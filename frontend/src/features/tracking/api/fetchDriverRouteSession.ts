...
export async function fetchDriverRouteSession(driverId: string): Promise<DriverRouteSession> {
  const response = await authFetch(`${API_URL}/api/admin/drivers/${encodeURIComponent(driverId)}/route-session`);
