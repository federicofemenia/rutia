...
import { UpdateCompanyRegistration } from './UpdateCompanyRegistration.js';
...
test('deshabilita el registro y persiste el cambio', async () => {
  const companyRepository = new StubCompanyRepository(new Map([[EXISTING_COMPANY.id, EXISTING_COMPANY]]));
  const useCase = new UpdateCompanyRegistration(companyRepository);

  const result = await useCase.execute('company-1', false);

  assert.equal(result?.registrationEnabled, false);
  assert.equal(companyRepository.updated?.registrationEnabled, false);
});

test('devuelve null si la empresa no existe', async () => {
  const useCase = new UpdateCompanyRegistration(new StubCompanyRepository(new Map()));
